...
export class CouponService {
  ...
  async updateCoupon(
    id: number,
    couponData: Partial<Omit<Coupon, "details" | "conditions">> & {
      details?: Partial<Coupon["details"]>;
  ...
  async getApplicableCoupons(cart: any) {
    const couponRepository = AppDataSource.getRepository(Coupon);
    ...
    for (const coupon of coupons) {
      ...
      if (coupon.type === "cart-wise") {
        ...
        cart.items.forEach(
          (item: { productId: number; price: number; quantity: number }) => {
        ...
        coupon.details.buyProducts?.forEach((buyProduct) => {
          const cartItem = cart.items.find(
            (item: { productId: number }) => item.productId === buyProduct.productId
          );
          if (cartItem) {
        ...
        if (applicableRepetitions > 0) {
          ...
          coupon.details.getProducts?.forEach((getProduct) => {
            const cartItem = cart.items.find(
              (item: { productId: number }) => item.productId === getProduct.productId
            );
            if (cartItem) {
  ...
  async applyCouponToCart(couponId: number, cart: any): Promise<any> {
    ...
    switch (coupon.type) {
      ...
      case "bxgy":
        if (
          Array.isArray(coupon.details.buyProducts) &&
          ...
          if (validForDiscount) {
            getProducts.forEach((getProduct) => {
              const getProductItem = cart.items.find(
                (item: { productId: number }) =>
                  item.productId === getProduct.productId
              );
              if (getProductItem) {
                const quantityEligibleForDiscount = Math.min(
                  getProductItem.quantity,
                  getProduct.quantity * repetitionLimit
                );
                const discountAmount =
                  quantityEligibleForDiscount * getProductItem.price;
