...
exports.sanitizeCouponMiddleware = sanitizeCouponMiddleware;
function sanitizeCouponMiddleware(req, res, next) {
    const { id } = req.params;
    const { cart } = req.body;
    ...
    couponDetails.buyProducts?.forEach((item, index) => {
    ...
    couponDetails.getProducts?.forEach((item, index) => {
        if (typeof item.productId !== "number" || item.productId <= 0) {
            return res.status(400).json({
                status: 400,
                success: false,
